  ...
  >
    ...
    >
      ...
      <mdui-linear-progress
        v-if="isLoading"
        style="position: absolute; left: 24px; top: 0; height: 4px; width: calc(100% - 48px);"
      ></mdui-linear-progress>
      ...
      <!-- 输入框滑动容器 -->
      <div class="input-slide-container">
        <!-- 用户名输入表单 -->
        <form
      ...
      >
        <mdui-button
          ...
          @click="step === 1 ? handleRegister() : handleBack()"
        >
          {{ step === 1 ? '注册账号' : '返回' }}
        </mdui-button>
      </div>
      ...
      <div style="position: absolute; right: 18px; bottom: 18px;">
        <mdui-button
          type="submit"
          @click="step === 1 ? handleNext() : handleLogin()"
        >
          {{ step === 1 ? '继续' : '登录' }}
        </mdui-button>
      </div>
...
// 绑定输入事件
const onUsernameInput = (e) => { username.value = e.target.value }
const onPasswordInput = (e) => { password.value = e.target.value }
